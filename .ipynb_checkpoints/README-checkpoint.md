...
# Run single evaluation (latest date in data)
python app.py

# Run for a specific date
python app.py --date 2026-01-28

# Run full backtest on all available data
python app.py --backtest

# Custom route and vessel options
python app.py --load-port Qatar --asia-port Tokyo --europe-port Rotterdam --vessel-class MEGI

# Skip risk analysis
python app.py --no-risk
...
|--------|-------------|---------|
| `--date` | Evaluation date (YYYY-MM-DD) | Latest date in data |
| `--backtest` | Run backtest on all historical data | False |
...
| `--no-risk` | Skip risk/stress analysis | False |
| `--no-save` | Don't save reports to files | False |